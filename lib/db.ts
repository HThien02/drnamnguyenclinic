...
export type ContactSettingsRow = { id: string; phone: string; whatsappPhone: string; whatsappMessage: string; instagramUrl: string; address: string; workingHours: string }

function mapContactSettings(data: any): ContactSettingsRow {
...
export async function getContactSettings(): Promise<ContactSettingsRow> {
  ...
  if (!data) throw new Error('Chưa có bản ghi thông tin liên hệ trong database.')
...
export async function updateContactSettings(settings: Omit<ContactSettingsRow, 'id'>): Promise<ContactSettingsRow> {
  ...
  const { data, error } = await supabase.from('contact_settings').update({ phone: settings.phone, whatsapp_phone: settings.whatsappPhone, whatsapp_message: settings.whatsappMessage, instagram_url: settings.instagramUrl, address: settings.address, working_hours: settings.workingHours, updated_at: new Date().toISOString() }).eq('id', current.id).select('*').single()
  if (error) throw error
  if (!data) throw new Error('Không tìm thấy bản ghi thông tin liên hệ để cập nhật.')
  return mapContactSettings(data)
}
