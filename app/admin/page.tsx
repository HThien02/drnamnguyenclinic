...
export default function AdminPage() {
  ...
  async function uploadServiceImage(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!response.ok || !data.success) throw new Error(data.error || 'Không thể tải ảnh dịch vụ lên.')
      setServiceForm((current) => ({ ...current, image: data.url }))
      setMessage('Đã tải ảnh dịch vụ lên. Hãy bấm Lưu để cập nhật database.')
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Không thể tải ảnh dịch vụ lên.')
    } finally {
  ...
  async function saveService(event: React.FormEvent) {
    ...
    const normalizedSlug = slugifyServiceName(serviceForm.slug || serviceForm.name)
    const payload = { ...serviceForm, slug: normalizedSlug, price: serviceForm.price === '' ? null : Number(serviceForm.price), displayOrder: Number(serviceForm.displayOrder), highlights: serviceForm.highlights.split('\n').map((value) => value.trim()).filter(Boolean), suitableFor: serviceForm.suitableFor.split('\n').map((value) => value.trim()).filter(Boolean), techniques: serviceForm.techniques.split('\n').map((value) => value.trim()).filter(Boolean), risksAndConsiderations: serviceForm.risksAndConsiderations.split('\n').map((value) => value.trim()).filter(Boolean) }
    if (!payload.name.trim() || !payload.slug || !payload.shortDescription.trim() || !payload.image.trim()) { setMessage('Vui lòng điền tên, mô tả ngắn và tải ảnh dịch vụ lên.'); return }
    setIsSavingService(true)
    try {
      const controller = new AbortController()
      const timeout = window.setTimeout(() => controller.abort(), 15000)
      const res = await fetch(editingServiceId ? `/api/services/${editingServiceId}` : '/api/services', { method: editingServiceId ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json', authorization: `Bearer ${sessionStorage.getItem('admin-token') || ''}` }, body: JSON.stringify(payload), signal: controller.signal })
      window.clearTimeout(timeout)
      const data = await res.json().catch(() => ({ success: false, error: 'Máy chủ trả về dữ liệu không hợp lệ.' }))
      if (!res.ok || !data.success) throw new Error(data.error || `Không thể lưu dịch vụ (HTTP ${res.status}).`)
      const saved = data.service as ServiceItem
      setServices((current) => editingServiceId ? current.map((item) => item.id === saved.id ? saved : item) : [...current, saved].sort((a, b) => a.displayOrder - b.displayOrder))
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {activeTab === 'services' && (
          <section className="mt-6 rounded-3xl border border-[#dce8f2] bg-white p-6 shadow-sm">
            ...
            {showServiceForm && <form onSubmit={saveService} className="mt-6 rounded-2xl border border-[#cfe2ee] bg-[#f6faff] p-4 sm:p-6">
              <div className="grid gap-5 lg:grid-cols-2">
                <div className="space-y-4 rounded-2xl border border-[#dce8f2] bg-white p-4">
                  <h3 className="border-b border-[#edf3f8] pb-3 text-sm font-bold text-[#0e3a63]">Thông tin cơ bản</h3>
                  <label className="block text-xs font-bold text-[#0e3a63]">Tên dịch vụ *<input required value={serviceForm.name} onChange={(e) => setServiceForm({ ...serviceForm, name: e.target.value })} className="mt-1.5 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label>
                  <label className="block text-xs font-bold text-[#0e3a63]">Slug URL *<input required value={serviceForm.slug} onChange={(e) => setServiceForm({ ...serviceForm, slug: e.target.value })} placeholder="vi-du-dich-vu" className="mt-1.5 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /><span className="mt-1 block text-[11px] font-normal text-[#66829a]">Có thể nhập tiếng Việt; hệ thống sẽ tự chuẩn hóa thành slug URL.</span></label>
                  <label className="block text-xs font-bold text-[#0e3a63]">Nhóm dịch vụ<select value={serviceForm.category} onChange={(e) => setServiceForm({ ...serviceForm, category: e.target.value as 'facial' | 'body' })} className="mt-1.5 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal"><option value="facial">Facial</option><option value="body">Body</option></select></label>
                  <label className="block text-xs font-bold text-[#0e3a63]">Mô tả ngắn *<textarea required value={serviceForm.shortDescription} onChange={(e) => setServiceForm({ ...serviceForm, shortDescription: e.target.value })} className="mt-1.5 min-h-24 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label>
                  <div className="rounded-xl border-2 border-dashed border-[#b7d4e5] p-3"><p className="text-xs font-bold text-[#0e3a63]">Ảnh dịch vụ</p><div className="mt-3 flex flex-wrap items-center gap-3"><div className="size-20 shrink-0 overflow-hidden rounded-lg bg-[#eef6fb]">{serviceForm.image ? <img src={serviceForm.image} alt="Xem trước ảnh dịch vụ" className="h-full w-full object-cover" /> : <ImageIcon className="m-6 size-8 text-[#8ca8be]" />}</div><label className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-[#0e5d94] px-3 py-2 text-xs font-bold text-white"><Upload className="size-4" /> Chọn ảnh từ máy<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" className="hidden" onChange={uploadServiceImage} disabled={isUploading} /></label></div></div>
                </div>
                <div className="space-y-4 rounded-2xl border border-[#dce8f2] bg-white p-4"><h3 className="border-b border-[#edf3f8] pb-3 text-sm font-bold text-[#0e3a63]">Nội dung chi tiết</h3><label className="block text-xs font-bold text-[#0e3a63]">Điểm nổi bật<textarea value={serviceForm.highlights} onChange={(e) => setServiceForm({ ...serviceForm, highlights: e.target.value })} className="mt-1.5 min-h-24 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label><label className="block text-xs font-bold text-[#0e3a63]">Phù hợp với<textarea value={serviceForm.suitableFor} onChange={(e) => setServiceForm({ ...serviceForm, suitableFor: e.target.value })} className="mt-1.5 min-h-24 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label><label className="block text-xs font-bold text-[#0e3a63]">Kỹ thuật thực hiện<textarea value={serviceForm.techniques} onChange={(e) => setServiceForm({ ...serviceForm, techniques: e.target.value })} className="mt-1.5 min-h-24 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label><label className="block text-xs font-bold text-[#0e3a63]">Lưu ý & rủi ro<textarea value={serviceForm.risksAndConsiderations} onChange={(e) => setServiceForm({ ...serviceForm, risksAndConsiderations: e.target.value })} className="mt-1.5 min-h-24 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label><label className="block text-xs font-bold text-[#0e3a63]">Tư vấn trước điều trị<textarea value={serviceForm.preConsultation} onChange={(e) => setServiceForm({ ...serviceForm, preConsultation: e.target.value })} className="mt-1.5 min-h-20 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label></div>
              </div>
              <div className="mt-5 grid gap-4 border-t border-[#dce8f2] pt-5 sm:grid-cols-3"><label className="block text-xs font-bold text-[#0e3a63]">Giá hiển thị<input value={serviceForm.price} onChange={(e) => setServiceForm({ ...serviceForm, price: e.target.value })} className="mt-1.5 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label><label className="block text-xs font-bold text-[#0e3a63]">Kiểu giá<select value={serviceForm.priceDisplayType} onChange={(e) => setServiceForm({ ...serviceForm, priceDisplayType: e.target.value as 'contact' | 'from' | 'fixed' })} className="mt-1.5 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal"><option value="contact">Liên hệ</option><option value="from">Từ mức giá</option><option value="exact">Giá cố định</option></select></label><label className="block text-xs font-bold text-[#0e3a63]">Thứ tự hiển thị<input value={serviceForm.displayOrder} onChange={(e) => setServiceForm({ ...serviceForm, displayOrder: e.target.value })} className="mt-1.5 w-full rounded-xl border border-[#c8dcea] px-3 py-2.5 text-sm font-normal" /></label></div><div className="mt-5 flex flex-wrap items-center justify-between gap-3"><label className="flex items-center gap-2 text-sm font-bold text-[#0e3a63]"><input type="checkbox" checked={serviceForm.isActive} onChange={(e) => setServiceForm({ ...serviceForm, isActive: e.target.checked })} /> Hiển thị dịch vụ</label><div className="flex gap-2"><button type="button" onClick={() => setShowServiceForm(false)} className="rounded-xl border border-[#c8dcea] px-4 py-2.5 text-sm font-bold text-[#66829a]">Hủy</button><button disabled={isSavingService || isUploading} className="rounded-xl bg-[#0e5d94] px-5 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-60">{isSavingService ? 'Đang lưu...' : 'Lưu dịch vụ'}</button></div></div>
            </form>}
        ...
        {activeTab === 'contact' && (
          <section className="mt-6 rounded-3xl border border-[#dce8f2] bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 rounded-2xl border border-[#dce8f2] bg-[#f6faff] p-5">
              ...
              <p className="mt-4 text-xs text-[#66829a]">Tất cả ảnh bác sĩ được thêm bằng nút tải ảnh từ máy và lưu vào Supabase Storage.</p>
            </div>
